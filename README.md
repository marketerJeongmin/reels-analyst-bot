...
이 봇은 크게 두 가지 기능으로 동작한다.

- 입력 → 결과: 분석 기능
- 대본입력 → 캡컷 / 인스타 / 유튜브: 원소스멀티유즈 기능
...
세 점수는 그대로 유지하고, 아래 지표는 `왜 이런 결과가 나왔는지` 해석하는 보조 진단으로 쓴다.

- 조회수 낮음: 관심 부족, 표지/썸네일 문제, 초반 3초 문제 가능성
- 좋아요 낮음: 공감 부족, "내 얘기다" 느낌 부족, 감성 터치 부족 가능성
- 댓글 낮음: 소통 불능, 참여 유도 부족, 질문 설계 부족 가능성
- 공유 낮음: 자극이나 정보 부족, 남에게 보내기 애매한 콘텐츠일 가능성
- 저장 낮음: 다시 볼 필요가 없고 가치가 부족한 휘발성 콘텐츠일 가능성
...
주간/월간 리포트도 같은 기준을 따라:

- 세 점수 추이
- 지표별 저조 진단
- 가장 약한 지표를 먼저 개선하는 액션
